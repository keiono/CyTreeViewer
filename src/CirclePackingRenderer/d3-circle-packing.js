...
const getColorMap = () =>
  d3Scale
    .scaleLinear()
    .domain([-1, 5])
    .range(['hsl(152,80%,80%)', 'hsl(228,50%,50%)'])
...
const CirclePacking = (tree, svgTree, size = 1200) => {
  ...
  const circle = g
    ...
    .style('fill', function(d) {
      return d.children ? colorMapper(d.depth) : 'rgba(255, 255, 255,0.5)'
    })
    .on('click', function(d) {
  ...
  svg.style('background', colorMapper()).on('click', function() {
    zoom(root)
  })
